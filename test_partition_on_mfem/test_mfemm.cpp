...
void read_mesh(MFEMMesh10 &mesh, const Mesh &mfem_mesh)
{
   int size = mfem_mesh.vertices.Size();
   mesh.vertices.resize(size);
   for (int i = 0; i < size; i++)
   {
      mesh.vertices[i].coords[0] = mfem_mesh.GetVertex(i)[0];
      mesh.vertices[i].coords[1] = mfem_mesh.GetVertex(i)[1];
      mesh.vertices[i].coords[2] = mfem_mesh.GetVertex(i)[2];
   }

   size = mfem_mesh.elements.Size();
   mesh.elements.resize(size);
   for (int i = 0; i < size; i++)
   {
      auto &x = mesh.elements[i];
      auto &y = mfem_mesh.elements[i];
      x.attribute = y->GetAttribute();
      x.geom_type = y->GetGeometryType();
      int len = y->GetNVertices();
      x.vertex_indices.resize(len);
      x.vertex_indices.assign(y->GetVertices(), y->GetVertices() + len);
   }
   size = mfem_mesh.boundary.Size();
   mesh.boundary.resize(size);
   for (int i = 0; i < size; i++)
   {
      auto &x = mesh.boundary[i];
      auto &y = mfem_mesh.boundary[i];
      x.attribute = y->GetAttribute();
      x.geom_type = y->GetGeometryType();
      int len = y->GetNVertices();
      x.vertex_indices.resize(len);
